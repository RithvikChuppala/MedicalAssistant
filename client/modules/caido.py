...
WORDS = ["YO","HE","CA\xcdDO"]


def isValid(text):
    return bool(re.search(r'\byo he ca\xcddo\b', text, re.IGNORECASE))
...
def sendSMS(mic, client, to_phone_number_one, to_phone_number_two, from_phone_number):
    client.messages.create(to=to_phone_number_one, from_=from_phone_number, body="Science Fair voice test run")
